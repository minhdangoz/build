...
| Desktop/VNC mặc định | ✅ LightDM auto-login XFCE trên Xorg (`Type=x11`). `x11vnc.service` dùng `-auth guess` để tìm Xauthority của display manager hiện hành, nên hoạt động với cả LightDM/XFCE và GDM/Xorg thay vì hard-code cookie GDM. |
