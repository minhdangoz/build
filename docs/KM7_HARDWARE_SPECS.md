...
- **WiFi**: **Amlogic W1, not Broadcom.** The DT node uses the generic
  ...
  A live GNOME image previously hit a `work_thread amlwifi` task permanently
  blocked in `mmc_wait_for_req_done` with 9,764 `fe088000.sdio` CMD53
  timeouts in roughly 50 minutes, at both 200 MHz and (after a downclock
  attempt) 50 MHz — so the bus clock was never the actual variable. The
