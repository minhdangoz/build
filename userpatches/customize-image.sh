...
InstallBrowsers() {
	# TV-box images should come with real browsers preinstalled, not just
	# GNOME Web. Chrome, Brave and Vivaldi all publish direct arm64 .deb
	# downloads, so grab those instead of standing up apt repos for each.
	export DEBIAN_FRONTEND=noninteractive
	local workdir
	workdir=$(mktemp -d)

	# Chrome publishes a stable "current" alias that always points at the
	# latest arm64 build.
	curl -fLo "${workdir}/google-chrome.deb" \
	...
	vivaldi_deb_url=$(curl -fsSL https://vivaldi.com/download/ \
		| grep -oE 'https://downloads\.vivaldi\.com/stable/vivaldi-stable_[^"]*arm64\.deb' | head -1)
	curl -fLo "${workdir}/vivaldi.deb" "${vivaldi_deb_url}"

	# Brave only ships versioned GitHub release assets, no static path --
	# resolve the arm64 asset off the latest release via the API.
	local brave_deb_url
	brave_deb_url=$(curl -fsSL https://api.github.com/repos/brave/brave-browser/releases/latest \
		| grep -oE '"browser_download_url": *"[^"]*/brave-browser_[^"]*_arm64\.deb"' \
		| head -1 | cut -d'"' -f4)
	curl -fLo "${workdir}/brave-browser.deb" "${brave_deb_url}"
